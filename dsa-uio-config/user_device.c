...
#define WQ_STATE_ENABLED 0x1
// currently there is only one wq on each dsa
#define WQ_IDX 0
...
#define GENSTS_DEV_STATE_ENABLED 0x01
...
static int
init_pci_device(struct udev_info *udi, uint64_t resource[1], bool read_only)
{
	struct idxd_pci_common *pci;
	...
	if(!read_only) {
		...
		if ((pci->regs->gensts & GENSTS_DEV_STATE_MASK) != GENSTS_DEV_STATE_ENABLED) {
			ERR("Device status is not enabled");
			rc = -errno;
			goto err;
		}
		// check wq enabled 
		if ((idxd_get_wq_cfg(pci, WQ_IDX)[WQ_STATE_IDX]>>WQ_STATE_SHIFT) != WQ_STATE_ENABLED) {
			ERR("WQ status is not enabled");
			rc = -errno;
			goto err;
		}
	}
...
static int
uio_init(struct udev_info *udi, bool read_only)
{
	int rc;
	...
	rc = init_pci_device(udi, &udi->resource[0], read_only);
	if (rc)
		return rc;
...
#define COUNT_CMD(f) \
	...
	"count=0; for dev in /sys/bus/pci/drivers/"#f"/*; do [ -L $dev ] || continue;"\
	"dev=$(basename $dev); [ ${dev} = 'module' ] && continue;"\
	"vendor=$(cat /sys/bus/pci/devices/${dev}/vendor);"\
	"device=$(cat /sys/bus/pci/devices/${dev}/device);"\
	"[ $vendor = 0x8086 ] && [ $device = 0x0b25 ] && count=$((count+1));"\
	" done; echo $count"
...
user_driver_init(struct tcfg *tcfg)
{
	...
	if (uio_cnt && vfio_cnt) {
		INFO("uio(%d) and vfio(%d) counts, only device under uio driver used now.\n",
			uio_cnt, vfio_cnt);
	}

	nb_udev = uio_cnt;
	if (nb_udev == 0) {
		ERR("zero uio driver devices detected\n");
		return -1;
	}
...
ud_wq_find(char *dname, int wq_id, int shared, int numa_node, bool read_only)
{
	struct udev_info *pd;
	...
	for (i = 0; i < nb_udev; i++) {
		...
		// skip if not the same device
		if (dname) {
			if (strcmp(dname, pd->bdf + strlen("0000:")))
				continue;
		}

		// skip if not the same numa node
		if ((!dname && numa_node != -1) && (numa_node != pd->numa_node))
			continue;
...
void *
ud_wq_get(char *dname, int wq_id, int shared, int numa_node, bool read_only)
{
	struct ud_wq_info *uwi;

	uwi = ud_wq_find(dname, wq_id, shared, numa_node, read_only);

	return uwi ? uwi->ptr : NULL;
}

bool ud_wq_get_next(struct UioWqInfo* info)
{
	struct ud_wq_info *uwi = NULL;
	uwi = ud_wq_find(NULL, -1, 0, -1, true);
	if (uwi) {
		info->bdf = uwi->udi->bdf;
		info->numa_node = uwi->udi->numa_node;
		info->portal = uwi->ptr;
		return true;
	}
	return false;
...
void ud_wq_unmap(void *ptr)
{
	...
	pq->ptr = NULL;
	// don't disable wq here
	// since we want to keep device and wq always enabled
}
