...
# This script is from Intel's QAT guide book.
# See https://www.intel.com/content/www/us/en/developer/topic-technology/open/quick-assist-technology/overview.html, Intel QAT Software for Linux—Programmer's Guide: Hardware v2.0.

# usage:
# 1. Open a terminal and enter this directory
# 2. Run "python3 monitor.py"
# 3. A terminal-based GUI will show the current status of the hardware,
#    which get refreshed every period of time.
#    You can have another terminal to run simple tests, and observe the 
#    hardware status here. Ctrl+C to exit.
        ...
        if "qat_dev" in output_adf[i]:
            ...
            if "up" == output_adf[i + 1]:
                if telemetry_supported == True:
                    ...
                    # Reset variables to ensure we only attempt to enable telemetry on devices that support telemetry and are in up state
                    state = "down"
                    name = None
                    telemetry_supported = False
        i += 1
    ...
    while True:
        try:
            ...
            for device in devices:
                ...
                window.addstr(4 + count, 10, device[0] + '\t' + compression + '\t' + str(decompress_utilization) +
                              '\t' + str(pke_utilization) + '\t' + str(cph_utilization) +
                              '\t' + str(ath_utilization) + '\t' + str(usc_utilization) + '\t' + latency)
                count += 1
            window.addstr(4 + count, 10, "=========================================================================")
            window.refresh()
            # change it to control the refresh rate
            time.sleep(0.1)
